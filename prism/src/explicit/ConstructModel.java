...
public class ConstructModel extends PrismComponent
{
	...
	public Model constructModel(ModelGenerator modelGen, boolean justReach) throws PrismException
	{
		...
		//List<Observation> obsList = new ArrayList<Observation> (modelGen.createObservationList());
		//System.out.println("obsList :: " + obsList.toString());
		List<Agent> agentList = new ArrayList<Agent> (modelGen.createAgentList());
		...
		if (!justReach) {
			...
			case DTMC:
				...
				break;
			case POMAS:
				modelSimple = pomas = new POMASSimple();
				pomas.setVarList(varList);
				//pomas.setObservationsList(obsList);
				pomas.setAgentsList(agentList);
				break;
			case CTMC:
		...
		for (State initState : modelGen.getInitialStates()) {
			System.out.println("\ninitState = " + initState.toString());
			explore.add(initState);
			states.add(initState);
			if (!justReach) {
				System.out.println("not just reach ...");
		...
		while (!explore.isEmpty()) {
			...
			System.out.println(">>> modelGen.getNumChioces :: " + nc);
			for (i = 0; i < nc; i++) {
				...
				System.out.println(">>> modelGen.getNumTransitions :: " + nt);
				for (j = 0; j < nt; j++) {
					...
					if (!justReach) {
						...
						case POMAS:
							System.out.println("\nsrc = " + src + ", dest = " + dest);
							System.out.println(">>> modelGen.getTransitionProbability :: " 
									+ modelGen.getTransitionProbability(i, j));
							System.out.println(">>> modelGen.getTransitionLabel :: " 
									+ modelGen.getTransitionLabel(i, j));
							pomas.addToProbability(src, dest, modelGen.getTransitionProbability(i, j), 
									modelGen.getAgent(i,j), modelGen.getTransitionLabel(i, j));
							break;
						case CTMC:
		...
		if (!justReach) {
			...
			case DTMC:
				...
				break;
			case POMAS:
				model = sort ? new POMASSimple(pomas, permut) : (POMASSimple) pomas;
				break;
			case CTMC:
