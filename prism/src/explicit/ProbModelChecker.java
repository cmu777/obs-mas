...
public class ProbModelChecker extends NonProbModelChecker
{
	...
	protected StateValues checkExpressionProb(Model model, ExpressionProb expr, boolean forAll, Coalition coalition, BitSet statesOfInterest) throws PrismException
	{
		...
		//System.out.println("#### checkExpressionProb.probs = " + probs.valuesL[0].getProb());
	...
	protected StateValues checkProbPathFormulaSimple(Model model, Expression expr, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		if (expr instanceof ExpressionTemporal) {
			...
			// Until
				...
				if (exprTemp.hasBounds()) {
					...
					//System.out.println("#### checkProbPathFormulaSimple.probs = " + probs.valuesL[0].getProb());
				}
		...
		if (negated) {
			...
			if (probs.valuesL != null)
				probs.negProbTraceListValue();
		}
	...
	/**
	 * Compute probabilities for an opacity operator.
	 */
	protected StateValues checkProbOpacity(Model model, ExpressionTemporal expr, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		} else if (expr.getOperator() == ExpressionTemporal.P_U) {
			probsProp = checkProbUntil(model, expr, minMax, statesOfInterest);
		} else
			throw new PrismNotSupportedException("Cannot model check " + expr + " for " + model.getModelType() + "s");
		
		StateValues probsNegProp = probsProp.deepCopy();
		probsNegProp.timesConstant(-1.0);
		probsNegProp.plusConstant(1.0);
		probsNegProp.negProbTraceListValue();
		...
		case POMAS:
			res = ((POMASModelChecker) this).computeOpacityProbs((POMAS) model, probsProp, probsNegProp);
			result.setStrategy(res.strat);
			return StateValues.createFromLabeledDoubleArray(res.solnWithLabels, res.solnDeadlocks, model);
		default:
			throw new PrismNotSupportedException("Cannot model check " + expr + " for " + model.getModelType() + "s");
		}
	}
	...
	protected StateValues checkProbNext(Model model, ExpressionTemporal expr, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		case DTMC:
			...
			//break;
		case POMAS:
			res = ((POMASModelChecker) this).computeNextProbs((POMAS) model, target);
			result.setStrategy(res.strat);
			return StateValues.createFromLabeledDoubleArray(res.solnWithLabels, res.solnDeadlocks, model);
			//break;
		case MDP:
	...
	protected StateValues checkProbBoundedUntil(Model model, ExpressionTemporal expr, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		if (windowSize == null) {
			...
			case DTMC:
				...
				break;
			case POMAS:
				res = ((POMASModelChecker) this).computeUntilProbs((POMAS) model, remain, target);
				result.setStrategy(res.strat);
				sv = StateValues.createFromLabeledDoubleArray(res.solnWithLabels, res.solnDeadlocks, model);
				break;
			case MDP:
			...
			case DTMC:
				...
				break;
			case POMAS:
				res = ((POMASModelChecker) this).computeBoundedUntilProbs((POMAS) model, remain, target, windowSize);
				break;
			case MDP:
		...
		if (lowerBound > 0) {
			...
			for (i = 0; i < lowerBound; i++) {
				...
				case DTMC:
					...
					break;
				case POMAS:
					probs = ((POMASModelChecker) this).computeRestrictedNext((POMAS) model, remain, probs);			
					break;
				case MDP:
	...
	protected Rewards constructRewards(Model model, RewardStruct rewStruct) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			rewards = constructRewards.buildMCRewardStructure((POMAS) model, rewStruct, constantValues);
			break;
		case MDP:
	...
	protected StateValues checkRewardInstantaneous(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			res = ((POMASModelChecker) this).computeInstantaneousRewards((POMAS) model, (MCRewards) modelRewards, t);
			break;
		case CTMC:
	...
	protected StateValues checkRewardCumulative(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			res = ((POMASModelChecker) this).computeCumulativeRewards((POMAS) model, (MCRewards) modelRewards, timeInt);
			break;
		case CTMC:
	...
	protected StateValues checkRewardTotal(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			res = ((POMASModelChecker) this).computeTotalRewards((POMAS) model, (MCRewards) modelRewards);
			break;
		case CTMC:
			break;
		case MDP:
	...
	protected StateValues checkRewardReach(Model model, Rewards modelRewards, ExpressionTemporal expr, MinMax minMax, BitSet statesOfInterest) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			res = ((POMASModelChecker) this).computeReachRewards((POMAS) model, (MCRewards) modelRewards, target);
			break;
		case CTMC:
	...
	protected StateValues checkSteadyStateFormula(Model model, Expression expr, MinMax minMax) throws PrismException
	{
		...
		case DTMC:
			...
			break;
		case POMAS:
			double multProbs1[] = Utils.bitsetToDoubleArray(b, model.getNumStates());
			res = ((POMASModelChecker) this).computeSteadyStateBackwardsProbs((POMAS) model, multProbs1);
			break;
		default:
