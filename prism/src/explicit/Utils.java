...
public class Utils
{
	...
	/**
	 * Create an n-element array of ProbTraceList (0s and 1s) from a BitSet.
	 *  @param bs The bitset specifying 0s and 1s
	 *  @param n The size of the array.
	 */
	public static ProbTraceList[] bitsetToTraceArray(BitSet bs, int n)
	{
		int i;
		ProbTraceList res[] = new ProbTraceList[n];
		for (i = 0; i < n; i++) {
			res[i] = new ProbTraceList(new ArrayList<ProbTransLabel>());
			res[i].setProb(bs.get(i) ? 1.0 : 0.0);
		}
		return res;
	}
