...
public class LTLModelChecker extends PrismComponent
{
	...
	public LTLProduct<POMAS> constructProductMC(ProbModelChecker mc, POMAS model, Expression expr, BitSet statesOfInterest, AcceptanceType... allowedAcceptance) throws PrismException
	{
		// Convert LTL formula to automaton
		Vector<BitSet> labelBS = new Vector<BitSet>();
		DA<BitSet,? extends AcceptanceOmega> da;
		...
		// Build product of model and automaton
		mainLog.println("\nConstructing MC-"+da.getAutomataType()+" product...");
		LTLProduct<POMAS> product = constructProductModel(da, model, labelBS, statesOfInterest);
		mainLog.print("\n" + product.getProductModel().infoStringTable());
	 ...
	 public <M extends Model> LTLProduct<M> constructProductModel(DA<BitSet,? extends AcceptanceOmega> da, M model, Vector<BitSet> labelBS, BitSet statesOfInterest) throws PrismException
	{
		...
		case POMAS: {
			POMASSimple POMASProd = new POMASSimple();
			POMASProd.setVarList(newVarList);
			prodModel = POMASProd;
			break;
		}
		case MDP: {
		...
		while (!queue.isEmpty()) {
			...
			for (int j = 0; j < numChoices; j++) {
				...
				case DTMC:
					...
					break;
				case POMAS:
					iter = ((POMAS) model).getTransitionsIterator(s_1);
					break;
				case MDP:
				...
				while (iter.hasNext()) {
					...
					case DTMC:
						...
						break;
					case POMAS:
						((POMASSimple) prodModel).setProbability(map[s_1 * daSize + q_1], map[s_2 * daSize + q_2], prob);
						break;
					case MDP:
					case STPG:
