...
public class ModulesFile extends ASTElement implements ModelInfo
{
	...
	// Lists of transition label info (name)
	//private List<Observation> tranLblObservations;
	//Lists of agents information (name, actions and observations)
	...
	public ModulesFile()
	{
		...
		initStates = null;
		//tranLblObservations = new Vector<Observation>();
	...
	{
		tranLblObservations.add(o);
	}
	
	public void setObservation(int i, Observation o)
	{
		tranLblObservations.set(i, o);
	}*/
	...
	// Transition label query methods

	/**
	 * Get the total number of transition labels.
	 */
	//public int getNumTranLbls()
	//{
		//return tranLblObservations.size();
	//}

	/**
	 * Look up the index of a transition label in the model by name.
	 * Returns -1 if there is no such transition label. 
	 */
	/*public int getTranLblIndex(String name)
	{
		for (int i=0; i<tranLblObservations.size(); i++) {
			if (tranLblObservations.get(i).getName().equals(name)){
				return i;
			}				
		}
		return -1;
	...
	/**
	 * Get the name of the ith transition label.
	 */
	/*public Observation getTranLblObservations(int i)
	{
		return tranLblObservations.get(i);
	}

	public List<Observation> getTranLblObservations()
	{
		return tranLblObservations;
	}
	
	public String getObservationByLabel(String name)
	{
		for (int i=0; i<tranLblObservations.size(); i++) {
			if (tranLblObservations.get(i).getName().equals(name)){
				return tranLblObservations.get(i).getObservedLbl();
			}				
		}
		return null;
	}
	
	public int getNumTranLblObservations()
	{
		return tranLblObservations.size();
	}*/
	...
	//Agent query methods
		...
		/**
		 * Look up the index of a agent in the model by name.
		 * Returns -1 if there is no such agent. 
		 */
		...
		public List<Observation> getActionsByAgent(String name)
		{
			for (int i=0; i<agents.size(); i++) {
				if (agents.get(i).getName().equals(name)){
					return agents.get(i).getObservationsList();
				}				
			}
			return null;
		}
		
		public int getAgentAction(String ag, String lb)
		{
			for (int i=0; i<agents.size(); i++) {
				if (agents.get(i).getName().equals(ag)){
					//return agents.get(i).getObservationsList();
					List<Observation> obs = agents.get(i).getObservationsList();
					for (int j=0; j<obs.size(); j++) {
						if (obs.get(j).getName().equals(lb))
							return 1;
					}
					return -1;
				}				
	 ...
	 * Also performs various syntactic checks on the variables.   
	 */
	/*public TransitionLabelList createTranLblList() throws PrismLangException
	{
		return new TransitionLabelList(this);
	}*/
	...
	public ASTElement deepCopy()
	{
		...
		//ret.tranLblObservations = (tranLblObservations == null) ? null : new ArrayList<Observation>(tranLblObservations);
		ret.agents = (agents == null) ? null : new ArrayList<Agent>(agents);
