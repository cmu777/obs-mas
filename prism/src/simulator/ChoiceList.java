...
public class ChoiceList implements Choice
{
	...
	protected List<String> transLabel; // for probabilistic labeled transitions
	...
	public ChoiceList(int n)
	{
		...
		transLabel = new ArrayList<String>(n);
		agent = new ArrayList<String>(n);
	...
	public void addTransLabel(String label)
	{
		this.transLabel.add(label);
	}
	...
	public void setTransLabel(int i, String label)
	{
		if (i < 0 || i >= size())
			return;
		this.transLabel.set(i, label);
	}
	
	public void setAgent(int i, String ag)
	{
		if (i < 0 || i >= size())
			return;
		this.agent.set(i, ag);
	...
	{
		return getTransLabel(0);
	}
	
	public String getTransLabel(int i)
	{
		if (i < 0 || i >= size())
			return null;
			//throw new PrismLangException("Invalid grouped transition index " + i);
		return transLabel.get(i);
	}
