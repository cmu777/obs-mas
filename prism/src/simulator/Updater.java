...
public class Updater extends PrismComponent
{
	...
	//protected List<Observation> obsList;
	//protected List<String> tranLblList; 
	...
	public Updater(ModulesFile modulesFile, VarList varList, PrismComponent parent)
	{
		...
		//obsList = modulesFile.getTranLblObservations();
	...
	public void calculateTransitions(State state, TransitionList transitionList) throws PrismException
	{
		...
		//System.out.println("transitionList : " + transitionList.getNumTransitions());
		// Clear lists/bitsets
	...
	private ChoiceListFlexi processUpdatesAndCreateNewChoice(int moduleOrActionIndex, Updates ups, State state) throws PrismLangException
	{
		...
		for (i = 0; i < n; i++) {
			...
			a = ups.getAgentInState(i, state);
			l = ups.getTransLabelInState(i, state);
			//l = ups.getT
			...
			//Check if the agent, label and its observation declared
			if ((modulesFile.getModelType() == ModelType.POMAS) &&
					(a.length()>0) && (l.length()>0) &&
					(modulesFile.getAgentAction(a,l) < 0) ) {
				String s = "Transition label \"" + l + " of agent " + a + "\" (and its observation) is not declared.";
				throw new PrismLangException(s, ups);
			}
