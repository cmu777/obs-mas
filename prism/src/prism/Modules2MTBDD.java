...
public class Modules2MTBDD
{
	...
	// type
	private ModelType modelType;				// model type (dtmc/mdp/ctmc/POMAS)
	// modules
	...
	public Model translate() throws PrismException
	{
		...
		// get transition label and observation info
		//observations = modulesFile.getTranLblObservations();
		//System.out.println("observation list = " + observations.toString());
		agents = modulesFile.getAgents();
	...
	private void translateModules() throws PrismException
	{
		...
		if (modulesFile.getSystemDefn() == null) {
			...
			for (i = 0; i < numModules; i++) {
				...
				System.out.println("sys adding operand .." + moduleNames[i].toString());
			}
...
//		}
		...
		if ( (modelType == ModelType.DTMC) || (modelType == ModelType.POMAS)) {
	...
	private SystemDDs translateSystemDefnRec(SystemDefn sys, int[] synchMin) throws PrismException
	{
		...
		if (sys instanceof SystemModule) {
			System.out.println("system module...");
	...
	private SystemDDs translateSystemModule(SystemModule sys, int[] synchMin) throws PrismException
	{
		...
		System.out.println("Translate module ---" + module.toString());
	...
	private ComponentDDs translateModule(int m, Module module, String synch, int synchMin) throws PrismException
	{
		...
		for (l = 0; l < numCommands; l++) {
			...
			if (match) {
				...
				if (guardDDs[l].equals(JDD.ZERO)) {
					...
					//lblDDs[l] = JDD.Constant(0);
					//rewDDs[l] = JDD.Constant(0);
				}
				else {
					// translate updates, transition labels and do some checks on probs/rates
		...
		else if (modelType == ModelType.POMAS) {
			System.out.println("**************");
			compDDs = combineCommandsLbLProb(m, numCommands, guardDDs, upDDs);
		}
		else if (modelType == ModelType.MDP) {
	...
	private ComponentDDs combineCommandsProb(int m, int numCommands, JDDNode guardDDs[], JDDNode upDDs[])
	{
		...
		System.out.println("compdds.trans = " + compDDs.trans.toString());
		compDDs.min = 0;
		compDDs.max = 0;
	...
	// go thru guard/updates dds for all commands of a prob. labeled module and combine
	// also check for any guard overlaps, etc...
		
	private ComponentDDs combineCommandsLbLProb(int m, int numCommands, JDDNode guardDDs[], JDDNode upDDs[])
	{
		ComponentDDs compDDs;
		int i;
		JDDNode covered, transDD, tmp;
	...
	private JDDNode translateUpdates(int m, int l, Updates u, boolean synch, JDDNode guard) throws PrismException
	{
		int i, n;
		Expression p;
		ExpressionTransitionLabel lbl;
		JDDNode dd, udd, pdd, ldd;
		boolean warned;
		String msg;
		...
		dd = JDD.Constant(0);
		n = u.getNumUpdates();
		System.out.println("num of updates = " + n);
		for (i = 0; i < n; i++) {
