...
public class ProbModel implements Model
{
	...
	//protected JDDNode transLabels[]; // dds for transition labels (LDTMCs)
	protected JDDNode transLabels; 
	...
	public JDDNode getTransLabels()
	{
		return transLabels;
	}
	
	//public JDDNode getTransLabels(int i)
	//{
	//	return transLabels[i];
	//}
			...
			JDDVars[] mrv, JDDVars[] mcv, int nv, VarList vl, JDDVars[] vrv, JDDVars[] vcv, Values cv)
	{
		...
		// action / transition label info (optional) is initially null
		transActions = null;
		transPerAction = null;
		transLabels = null;
	...
	/**
	 * Set the DDs used to store transition action label indices (LDTMCs).
	 */
	public void setTransLabels(JDDNode transLabels)
	{
		this.transLabels = transLabels;
	}
	
	/**
	 * Set the DDs used to store transition action label indices (LDTMCs).
	 */
	/*public void setTransLabels(int i, JDDNode transLabel)
	{
		this.transLabels[i] = transLabel;
	}*/
	...
	public void filterReachableStates()
	{
		...
		/*if (transLabels != null) {
			for (i = 0; i < numStates + 1; i++) {
				JDD.Ref(reach);
				transLabels[i] = JDD.Apply(JDD.TIMES, reach, transLabels[i]);
				JDD.Ref(reach);
				tmp = JDD.PermuteVariables(reach, allDDRowVars, allDDColVars);
				transLabels[i] = JDD.Apply(JDD.TIMES, tmp, transLabels[i]);
			}
		}*/
		if (transLabels != null) {
			JDD.Ref(reach);
			transLabels = JDD.Apply(JDD.TIMES, reach, transLabels);	
		}
	...
	public void clear()
	{
		...
		if (transLabels != null) {
			/*for (int i = 0; i < numStates +1; i++) {
				JDD.Deref(transLabels[i]);
			}*/
			JDD.Deref(transLabels);
		}
	}
}
