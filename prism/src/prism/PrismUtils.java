...
public class PrismUtils
{
	...
	public static boolean doublesAreClose(ProbTraceList l1[], ProbTraceList l2[], double epsilon, boolean abs)
	{
		int i, n;
		n = Math.min(l1.length, l2.length);
		if (abs) {
			for (i = 0; i < n; i++) {
				if (!PrismUtils.doublesAreCloseAbs(l1[i].getProb(), l2[i].getProb(), epsilon))
					return false;
			}
		} else {
			for (i = 0; i < n; i++) {
				if (!PrismUtils.doublesAreCloseRel(l1[i].getProb(), l2[i].getProb(), epsilon))
					return false;
			}
		}
		return true;
