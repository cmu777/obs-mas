...
public class PrismCL implements PrismModelListener
{
	...
	private boolean exportlabeledtrans = false;
	...
	private String exportLabeledTransFilename = null;
	...
	public void go(String[] args) {
		try {
			...
			mainLog.println("\nTotal time for computation: " + l / 1000.0 + " seconds.");
	...
	private void doExports()
	{
		...
		// export labeled transition matrix to a file
		if (exportlabeledtrans) {
			try {
				File f = (exportLabeledTransFilename.equals("stdout")) ? null : new File(exportLabeledTransFilename);
				prism.exportLabeledTransToFile(exportordered, exportType, f);
			}
			// in case of error, report it and proceed
			catch (FileNotFoundException e) {
				error("Couldn't open file \"" + exportLabeledTransFilename + "\" for output");
			} catch (PrismException e) {
				error(e.getMessage());
			}

			if (exportPlainDeprecated)
				mainLog.printWarning("The -exportplain switch is now deprecated. Please use -exportlabeledtrans in future.");
		}
	...
	private void parseArguments(String[] args) throws PrismException
	{
		...
		for (i = 0; i < args.length; i++) {
			...
			if (args[i].length() > 0 && args[i].charAt(0) == '-') {
				...
				// override model type to POMAS
				else if (sw.equals("pomas")) {
					typeOverride = ModelType.POMAS;
				}
				...
				// export labeled transition matrix to file
				else if (sw.equals("exportlabeledtrans")) {
					if (i < args.length - 1) {
						exportlabeledtrans = true;
						exportLabeledTransFilename = args[++i];
					} else {
						errorAndExit("No file specified for -" + sw + " switch");
					}
				}
	...
	private void printHelp()
	{
		...
		mainLog.println("-exportlabeledtrans <file> ..... Export labeled transition matrix to a file to a file");
